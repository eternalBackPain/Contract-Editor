...
import ContractsLexer from './formal-grammar/ContractsLexer'
import ContractsParser from './formal-grammar/ContractsParser'
import ContractsParserListener from './formal-grammar/ContractsParserListener'

export function parseToXML(text) {
  ...
  class CustomListener extends ContractsParserListener {
    ...
    enterHeading4(ctx) {
      ...
      this.XMLOutput.push('<HeadingLevel4>' + heading.substring(5) + '</HeadingLevel4>')
    }
    exitHeading4(ctx) {}

    enterHeading3(ctx) {
      const heading = ctx.getChild(0).getText()
      this.XMLOutput.push('<HeadingLevel3>' + heading.substring(4) + '</HeadingLevel3>')
    }
    exitHeading3(ctx) {}

    enterHeading2(ctx) {
      const heading = ctx.getChild(0).getText()
      this.XMLOutput.push('<HeadingLevel2>' + heading.substring(3) + '</HeadingLevel2>')
    }
    exitHeading2(ctx) {}

    enterHeading1(ctx) {
      const heading = ctx.getChild(0).getText()
      this.XMLOutput.push('<HeadingLevel1>' + heading.substring(2) + '</HeadingLevel1>')
    }
    exitHeading1(ctx) {}
